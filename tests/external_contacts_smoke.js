...
(async () => {
  ...
  try {
    ...
    assert.match(result.categoryGuide.text, /Prepare o projeto/);
    assert.match(result.categoryGuide.text, /Comece com o bloco Preparar contatos/);
    assert.match(result.categoryGuide.text, /massinha condutiva/);
    assert.doesNotMatch(result.categoryGuide.text, /Repetir para sempre/);
    assert.match(result.categoryGuide.text, /LED externo, servo ou DHT11/);
    assert.match(result.categoryGuide.text, /Na BitDogLab V7, o Display usa as Conexões 2 e 3/);
    assert.doesNotMatch(result.categoryGuide.text, /AHT20/);
    assert.doesNotMatch(result.categoryGuide.text, /MPU6050/);
