# Grafico tela dividida - Temperatura em cima, Umidade embaixo
# Plota dois graficos simultaneos do Sensor 1
# Resultado esperado: metade de cima mostra temperatura, metade de baixo mostra umidade
...
_bufs = {}
def plot_grafico(buf_id, valor, pos, titulo):
    if buf_id not in _bufs:
        _bufs[buf_id] = []
    buf = _bufs[buf_id]
    buf.append(float(valor))
...
while True:
    ...
    umid = d[1] if d[1] is not None else 0
    plot_grafico("temp", temp, 1, "Temp1:" + str(round(temp, 1)))
    plot_grafico("umid", umid, 2, "Umid1:" + str(round(umid, 1)))
    time.sleep_ms(500)
