# Grafico com soma de temperaturas de dois sensores
# Plota Temp Sensor 1 + Temp Sensor 2 na tela toda
# Resultado esperado: grafico mostrando a soma das duas temperaturas
# Necessita dois sensores AHT20 conectados (Canal 0 e Canal 1)
...
"-":[0,0,7,0,0],"S":[7,4,7,1,7],"o":[0,0,7,5,7],"m":[0,5,7,7,5],
"a":[0,0,7,5,7],"T":[7,2,2,2,2],"e":[7,5,7,4,7],"p":[7,5,7,4,4],
" ":[0,0,0,0,0]}
def _dc(x,y,c):
    g=_ft.get(c,_ft[" "])
    for r in range(5):
...
while True:
    d1 = aht_esq.get_data() if aht_esq.is_ready else (None, None)
    d2 = aht_dir.get_data() if aht_dir.is_ready else (None, None)
    t1 = d1[0] if d1[0] is not None else 0
    t2 = d2[0] if d2[0] is not None else 0
    soma = t1 + t2
    plot_grafico(soma, "SomaTemp:" + str(round(soma, 1)))
    time.sleep_ms(500)
