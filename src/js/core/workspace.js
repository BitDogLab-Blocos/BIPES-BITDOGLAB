...
WorkspaceManager.PIANO_NOTES = [
  { label: 'C', name: 'Dó', type: 'nota_do', frequency: 262, color: '#EA2027', left: 0 },
  { label: 'D', name: 'Ré', type: 'nota_re', frequency: 294, color: '#EE5A24', left: 1 },
  { label: 'E', name: 'Mi', type: 'nota_mi', frequency: 330, color: '#FFC312', left: 2 },
  { label: 'F', name: 'Fá', type: 'nota_fa', frequency: 349, color: '#C4E538', left: 3 },
  { label: 'G', name: 'Sol', type: 'nota_sol', frequency: 392, color: '#12CBC4', left: 4 },
  { label: 'A', name: 'Lá', type: 'nota_la', frequency: 440, color: '#833471', left: 5 },
  { label: 'B', name: 'Si', type: 'nota_si', frequency: 494, color: '#FD7272', left: 6 }
];

WorkspaceManager.PIANO_BLACK_KEYS = [
...
WorkspaceManager.showInteractivePiano = function() {
  ...
  WorkspaceManager.PIANO_NOTES.forEach(function(note, index) {
    ...
    WorkspaceManager.bindPianoKey(key, function() {
  ...
  WorkspaceManager.PIANO_BLACK_KEYS.forEach(function(note) {
    var key = document.createElement('button');
    key.type = 'button';
...
WorkspaceManager.bindPianoKey = function(key, action) {
...
WorkspaceManager.playPianoNote = function(frequency) {
  var AudioContext = window.AudioContext || window.webkitAudioContext;
  if (!AudioContext) return;

  var context = WorkspaceManager.audioContext || new AudioContext();
  WorkspaceManager.audioContext = context;
...
Code.playPianoNote = WorkspaceManager.playPianoNote;
